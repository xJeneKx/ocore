...
test.cb.serial('trying to modify a var conditionally frozen in an earlier formula', t => {
	...
	aa_composer.dryRunPrimaryAATrigger(trigger, address, aa, (arrResponses) => {
		...
		t.deepEqual(arrResponses[0].response_unit, null);
		t.deepEqual(arrResponses[0].objResponseUnit, null);
		t.deepEqual(arrResponses[0].response.error, {message:"variable x is frozen",formatedContext:"$x",codeLines:[{lineNumber:2,formula:"$x.b = 8;"}],xpath:"/messages/0/state"});
		fixCache();
		t.deepEqual(storage.assocUnstableUnits, old_cache.assocUnstableUnits);
		t.deepEqual(storage.assocStableUnits, old_cache.assocStableUnits);
		t.deepEqual(storage.assocUnstableMessages, old_cache.assocUnstableMessages);
...
test.cb.serial('invalid selector', t => {
	...
	validateAA(aa, async err => {
		...
		aa_composer.dryRunPrimaryAATrigger(trigger, aa_address, aa, (arrResponses) => {
			t.deepEqual(arrResponses.length, 1);
			t.deepEqual(arrResponses[0].bounced, true);
			t.deepEqual(arrResponses[0].response.error, {message:"result of [\"dictionary\",[[\"a\",\"9\"]]] is not a string or number: {\"a\":9}",formatedContext:"$x",codeLines:[{lineNumber:3,formula:"$x[{a:9}] = 1;"}],xpath:"/init"});
			fixCache();
			t.deepEqual(storage.assocUnstableUnits, old_cache.assocUnstableUnits);
			t.deepEqual(storage.assocStableUnits, old_cache.assocStableUnits);
			t.deepEqual(storage.assocUnstableMessages, old_cache.assocUnstableMessages);
...
test.cb.serial('invalid local var name', t => {
	...
	validateAA(aa, async err => {
		...
		aa_composer.dryRunPrimaryAATrigger(trigger, aa_address, aa, (arrResponses) => {
			t.deepEqual(arrResponses.length, 1);
			t.deepEqual(arrResponses[0].bounced, true);
			t.deepEqual(arrResponses[0].response.error, {message:"local var name must be string, evaluated to {\"obj\":{\"valueOf\":false,\"toString\":false},\"frozen\":false} (object)",formatedContext:"$dictionary",codeLines:[{lineNumber:2,formula:"$y = ${{valueOf: false, \"toString\": false}};"}],xpath:"/init"});
			fixCache();
			t.deepEqual(storage.assocUnstableUnits, old_cache.assocUnstableUnits);
			t.deepEqual(storage.assocStableUnits, old_cache.assocStableUnits);
			t.deepEqual(storage.assocUnstableMessages, old_cache.assocUnstableMessages);
...
test.cb.serial('invalid state var name', t => {
	...
	validateAA(aa, async err => {
		...
		aa_composer.dryRunPrimaryAATrigger(trigger, aa_address, aa, (arrResponses) => {
			t.deepEqual(arrResponses.length, 1);
			t.deepEqual(arrResponses[0].bounced, true);
			t.deepEqual(arrResponses[0].response.error, {message: "assignment: state var name must be string, dictionary,valueOf,false,toString,false evaluated to {\"obj\":{\"valueOf\":false,\"toString\":false},\"frozen\":false} (object)",formatedContext:"var[dictionary]",codeLines:[{"lineNumber":2,"formula":"var[{valueOf: false, \"toString\": false}] = 1;"}],xpath:"/messages/0/state"});
			fixCache();
			t.deepEqual(storage.assocUnstableUnits, old_cache.assocUnstableUnits);
			t.deepEqual(storage.assocStableUnits, old_cache.assocStableUnits);
			t.deepEqual(storage.assocUnstableMessages, old_cache.assocUnstableMessages);
